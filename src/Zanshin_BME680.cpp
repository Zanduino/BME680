...
bool BME680_Class::commonInitialization() 
{
  ...
  */
  ...
  if (_I2CAddress==0 && bitRead(SPI_Register,BME680_SPI_MEM_PAGE_POSITION)) { // Wrong mode for ID
...
uint8_t BME680_Class::setOversampling(const uint8_t sensor, const uint8_t sampling) {
  ...
  */
  ...
  void BME680_Class::readSensors(const bool waitSwitch) {
    ...
    var2 =
        ((int32_t)_H2 * (((temp_scaled * (int32_t)_H4) / ((int32_t)100)) +
    ...
    uint64_t uvar2;
    var1 =
        (int64_t)((1340 + (5 * (int64_t)_rng_sw_err)) * ((int64_t)lookupTable1[gas_range])) >> 16;
